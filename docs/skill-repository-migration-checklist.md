...
- [x] 由共享 style spec 生成并校验基础版和 `with-docs` 版 `SKILL.md` 中的视觉规则投影；两者发布时彼此独立，不依赖运行时链式调用。
- [x] 将独立的 `SKILL.md` 和生成索引提交到仓库，供 marketplace、`npx skills add` 和 `curl` 直接使用。
...
- [x] 文章源只读；默认在对话中输出，只有用户明确提供输出路径时才写文件（四个 with-docs 技能均声明并由 registry 测试锁定）。
