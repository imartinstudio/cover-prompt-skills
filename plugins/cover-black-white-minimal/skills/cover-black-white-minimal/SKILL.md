...
If generating the image directly, keep all semantic analysis internal and output only the generated image result. If the user sets `--out-type prompt`, output only the final image prompt. If the user sets `--out-type template` or omits `--out-type`, output only the template described in "Template Output" and do not output a generic image prompt.
...
```text
使用 $cover-black-white-minimal 生成一张封面
主题词：{topic}
副标题：{subtitle}
画幅比例：{ratio}
语言：{language}
